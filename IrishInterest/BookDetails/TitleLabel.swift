...
struct TitleLabel {
    ...
    func display(in contentView: UIView, with stack: VStack, using border: CGFloat) {
        let title = UILabel()
        let value = UILabel()
        UI.format(.subheadline, color: .secondaryLabel, label: title, text: titleText, nrOfLines: 1)
        UI.format(.subheadline, color: .secondaryLabel, label: value, text: valueText, nrOfLines: 1)
